  ...
  FormValidator.prototype = {
    ...
    tests: {
      sameAsPlaceholder: function (field, data) {
        if (field.getAttribute('placeholder')) return data.value != field.getAttribute('placeholder') || this.texts.empty;
        else return true;
      },
    ...
    prepareFieldData: function (field) {
      ...
      this.data[id].lengthRange = field.getAttribute('data-validate-length-range')
    ...
    extend: function (target, varArgs) {
      ...
      for (index = 1; index < arguments.length; index++) {
        ...
        if (nextSource != null)
          ...
          for (nextKey in nextSource) if (Object.prototype.hasOwnProperty.call(nextSource, nextKey)) to[nextKey] = nextSource[nextKey];
      }
    ...
    checkField: function (field, silent) {
      ...
      if (field.type != 'hidden' && !field.clientWidth) return { valid: true, error: '' };
      ...
      if (data.valid && data.validateLinked) {
        if (data['validateLinked'].indexOf('#') == 0) linkedTo = document.body.querySelector(data['validateLinked']);
        else if (form.length) linkedTo = form.querySelector('[name=' + data['validateLinked'] + ']');
